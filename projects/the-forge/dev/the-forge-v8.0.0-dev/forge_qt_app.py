...
class ForgeMainWindow(QMainWindow):
    ...
    def _build_mapping_section(self):
        ...
        # --- Attribute flag checkbox ---
        self.attr_first_checkbox = QCheckBox(
            "Place attribute rows first in each parent structure (Excel output)."
        )
        self.attr_first_checkbox.setChecked(False)
        self.attr_first_checkbox.setStyleSheet("color: #fff; font-weight: bold;")
        layout.addWidget(self.attr_first_checkbox)
        ...
        self.log_text.setStyleSheet("background: #BECACC;")
    ...
    def _on_generate(self):
        ...
        try:
            ...
            for msg_name, src_full_rows in src_messages.items():
                ...
                for col in range(tgt_level_start + max_tgt_level - 1, tgt_level_start - 1, -1):
                    ...
                    if col > tgt_level_start and all((ws.cell(row=row, column=col).value in (None, '')) for row in range(3, ws.max_row + 1)):
                        ws.delete_cols(col)
            wb.save(out)
            self._log(f"[SUCCESS] Output file created: {out}", level="success")
            # --- Post-processing QA: Excel Output Validator ---
            xsd_path = src if src else tgt
            try:
                from microservices.excel_output_validator import validate_excel_output, _log_messages
                _log_messages.clear()
            ...
            if hasattr(self, 'attr_first_checkbox') and self.attr_first_checkbox.isChecked():
                try:
                    from microservices.reorder_excel_attributes import reorder_attributes_in_excel
                    reorder_attributes_in_excel(out)
                    self._log("[INFO] Reordered attributes to appear first in each parent structure.", level="info")
                except Exception as e:
                    self._log(f"[ERROR] Failed to reorder attributes: {e}", level="error")
            self._finish_generate()
            # Clear output path field and variable to release file reference
            self.output_path.clear()
            self.output_path_full = ''
        except Exception as e:
            import traceback
    ...
    def _log(self, msg, level="info", widget=None):
        from PySide6.QtGui import QColor
        if widget is None:
            widget = self.log_text
        # Safety: only update if widget is valid, not deleted, and visible
        if widget is None or not isValid(widget) or not widget.isVisible():
            return
        if level == "error":
            widget.setTextColor(Qt.red)
        elif level == "success":
            widget.setTextColor(QColor("#1E7D36"))
        elif level == "warning":
            widget.setTextColor(QColor("#FFA500"))  # Orange
        elif level == "info" or level == "validate":
            widget.setTextColor(QColor("#263CC8"))  # EDP cobalt blue
        else:
    ...
    def _build_wsdl2xsd_section(self):
        ...
        self.wsdl2xsd_log_text.setStyleSheet("background: #BECACC;")
        layout.addWidget(self.wsdl2xsd_log_text)
        # layout.addStretch()  # Remove excessive stretch
        return w

    def log_to_wsdl_ui(self, message: str):
        # Determine log level by prefix
        ...
        if hasattr(self, 'wsdl2xsd_log_text') and self.wsdl2xsd_log_text:
            self._log(message, level, widget=self.wsdl2xsd_log_text)
