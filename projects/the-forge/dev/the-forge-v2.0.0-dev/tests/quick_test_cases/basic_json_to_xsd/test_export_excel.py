...
SRC_JSON = BASE_DIR / "source.json"
TGT_XSD = BASE_DIR / "target.xsd"
OUTPUT_XLSX = BASE_DIR / "exported_mapping.xlsx"

def main():
    # 1. Extract fields
    processor = SchemaProcessor()
    source_fields = processor.extract_fields_from_json_schema(str(SRC_JSON))
    target_fields = processor.extract_fields_from_xsd(str(TGT_XSD))
