...
class XSDParser:
    ...
    def parse_element(self, element, complex_types, simple_types, level=1, parent_path=None, req_param='Body', category='element'):
        ...
        if type_name and type_name in complex_types:
            ...
            # Handle sequence children
            seq = ct.find(XSD_NS+'sequence')
            if seq is not None:
                for child in seq.findall(XSD_NS+'element'):
                    rows.extend(self.parse_element(child, complex_types, simple_types, level+1, path, req_param, 'element'))
                for attr in seq.findall(XSD_NS+'attribute'):
                    rows.extend(self.parse_attribute(attr, path, req_param, complex_types, simple_types))
            # Handle direct attributes
            for attr in ct.findall(XSD_NS+'attribute'):
                rows.extend(self.parse_attribute(attr, path, req_param, complex_types, simple_types))
            # Handle simpleContent/complexContent extensions
            for sc_tag in [XSD_NS+'simpleContent', XSD_NS+'complexContent']:
                sc = ct.find(sc_tag)
                if sc is not None:
                    ext = sc.find(XSD_NS+'extension')
                    if ext is not None:
                        # Attributes in the extension
                        for attr in ext.findall(XSD_NS+'attribute'):
                            rows.extend(self.parse_attribute(attr, path, req_param, complex_types, simple_types))
        else:
            for ct in element.findall(XSD_NS+'complexType'):
                ...
                for attr in ct.findall(XSD_NS+'attribute'):
                    rows.extend(self.parse_attribute(attr, path, req_param, complex_types, simple_types))
                # Handle simpleContent/complexContent extensions for inline complexType
                for sc_tag in [XSD_NS+'simpleContent', XSD_NS+'complexContent']:
                    sc = ct.find(sc_tag)
                    if sc is not None:
                        ext = sc.find(XSD_NS+'extension')
                        if ext is not None:
                            for attr in ext.findall(XSD_NS+'attribute'):
                                rows.extend(self.parse_attribute(attr, path, req_param, complex_types, simple_types))
        for attr in element.findall(XSD_NS+'attribute'):
            rows.extend(self.parse_attribute(attr, path, req_param, complex_types, simple_types))
        return rows
