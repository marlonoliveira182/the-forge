...
def validate_excel_output(xsd_path: str, excel_path: str) -> None:
    if not PANDAS_AVAILABLE:
        log_to_ui("[WARNING] Pandas not available - skipping validation. Install pandas with: pip install pandas")
        return
    
    log_to_ui(f"[VALIDATE] Starting validation: XSD='{os.path.basename(xsd_path)}', Excel='{os.path.basename(excel_path)}'")
    ...
    log_to_ui(f"\n[VALIDATE] Validation completed: {total_errors} errors, {total_warnings} warning(s), {total_verified} fields verified across {len(message_fields)} messages.")
    ...
    if total_errors or total_warnings:
        try:
            ...
            log_to_ui(f"[WARNING] Could not export validation report: {e}")

if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        print("Usage: python excel_output_validator.py <xsd_path> <excel_path>")
        sys.exit(1)
    xsd_path = sys.argv[1]
    excel_path = sys.argv[2]
    def print_log(msg):
        print(msg)
    log_to_ui.__globals__['log_to_ui'] = print_log
    validate_excel_output(xsd_path, excel_path) 