...
def process_directory(directory):
    for filename in os.listdir(directory):
        ...
        if file_type == 'jsonschema':
            ...
            columns, rows = parse_xsd(filepath)
        elif file_type == '.xml':
            rows = parse_xml_example(filepath)
            columns = COLUMNS
        else:
            print(f'  Tipo de arquivo não suportado: {filename}')
            continue
        issues = double_check(rows, columns)
        output_file = os.path.join(TARGET_DIR, os.path.splitext(os.path.basename(filepath))[0] + '.xlsx')
        write_excel(rows, output_file, columns)
        print(f'  Excel gerado: {output_file}')
        if issues:
            print('  Double check report:')
            for issue in issues:
                print('   -', issue)
        else:
            print('  Double check: Nenhum problema encontrado.')
...
def detect_file_type(filename):
    ...
    if ext in SUPPORTED_TYPES:
        return ext
    # Try to detect JSON Schema vs JSON example
    if ext == '.json':
...
if __name__ == "__main__":
    print(f'Buscando arquivos em: {SOURCE_DIR}')
    process_directory(SOURCE_DIR)
    print(f'Buscando arquivos em: {TARGET_DIR}')
