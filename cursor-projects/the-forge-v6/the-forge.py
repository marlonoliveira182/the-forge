...
def extract_fields_from_json_schema(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        data = json.load(f)
    fields = []
    def walk(obj, levels=None, level=0, parent_required=None):
        if levels is None:
            levels = []
        if parent_required is None:
            parent_required = obj.get('required', []) if isinstance(obj, dict) else []
        if isinstance(obj, dict):
            # Se for array na raiz
            if obj.get('type') == 'array' and 'items' in obj:
                typ = 'array'
                desc = obj.get('description', '')
                min_items = obj.get('minItems', 0)
                max_items = obj.get('maxItems', 'unbounded' if 'maxItems' not in obj else obj['maxItems'])
                card = f"{min_items}..{max_items}"
                details = []
                if 'enum' in obj:
                    details.append(f"enum: {obj['enum']}")
                if 'pattern' in obj:
                    details.append(f"pattern: {obj['pattern']}")
                if '$comment' in obj:
                ...
                # Para itens do array, use o required do objeto items (se houver)
                item_required = obj['items'].get('required', []) if isinstance(obj['items'], dict) else []
                walk(obj['items'], levels, level+1, parent_required=item_required)
                return
            # Propriedades normais
            required_here = obj.get('required', [])
            for k, v in obj.get('properties', {}).items():
                lvls = levels + [k]  # Use original key (camelCase)
                typ = v.get('type', '')
                desc = v.get('description', '')
                # Cardinalidade
                card = '1' if k in required_here else '0..1'
                if typ == 'array':
                ...
                if 'exclusiveMinimum' in v:
                    details.append(f"exclusiveMinimum: {v['exclusiveMinimum']}")
                if 'exclusiveMaximum' in v:
                    details.append(f"exclusiveMaximum: {v['exclusiveMaximum']}")
                if 'format' in v:
                ...
                # Se for array, documentar estrutura interna
                if typ == 'array' and 'items' in v:
                    # Para itens do array, use o required do objeto items (se houver)
                    item_required = v['items'].get('required', []) if isinstance(v['items'], dict) else []
                    walk(v['items'], lvls, level+1, parent_required=item_required)
                elif typ == 'object' and 'properties' in v:
                    walk(v, lvls, level+1, parent_required=v.get('required', []))
    walk(data)
    return fields
...
def extract_fields_from_xsd(filepath, keep_case=False):
    ...
    def walk_element(element, levels=None, omit_root=False, parent_is_array=False):
        if levels is None:
            levels = []
        name = element.get('name', '')
        if not name:
        ...
        if is_array:
            ...
            if inner_elements:
                for inner in inner_elements:
                    # Não force minOccurs/maxOccurs dos filhos, use o valor real do XSD
                    walk_element(inner, lvls, omit_root=False, parent_is_array=True)
            else:
...
def run_xsd_to_jsonschema_operation(source_path, dest_dir):
    ...
    def parse_element(element):
        ...
        # Se for simpleType inline
            ...
            if 'fractionDigits' in details:
                unsupported.append(f"fractionDigits: {details['fractionDigits']}")
            if unsupported:
        ...
        # Se for simpleType referenciado
            ...
            if 'fractionDigits' in details:
                desc_parts.append(f"fractionDigits: {details['fractionDigits']}")
            if desc_parts:
...
def jsonschema_to_xsd(json_schema_path, dest_dir):
    ...
    def gen_xsd_element(name, obj, required_fields=None):
        ...
        if t == 'string':
            ...
            if '$comment' in obj:
                import re
                m = re.search(r'totalDigits\s*[:=]\s*(\d+)', obj['$comment'])
                if m:
                    restrictions.append(f'<xs:totalDigits value="{m.group(1)}"/>')
                m2 = re.search(r'fractionDigits\s*[:=]\s*(\d+)', obj['$comment'])
                if m2:
        ...
        if t == 'object':
            ...
            children = [gen_xsd_element(k, v, required) for k, v in props.items()]
            ...
            return f'<xs:element name="{to_pascal_case(name)}" minOccurs="{min_occurs}" maxOccurs="{max_occurs}">{annotation}' + gen_xsd_element(name, items) + '</xs:element>'
        elif restrictions:
            ...
            min_occurs = '1' if name in (required_fields or []) else '0'
            return f'<xs:element name="{to_pascal_case(name)}" type="xs:string" minOccurs="{min_occurs}" maxOccurs="1">{annotation}</xs:element>'
        elif t == 'integer':
            min_occurs = '1' if name in (required_fields or []) else '0'
            return f'<xs:element name="{to_pascal_case(name)}" type="xs:int" minOccurs="{min_occurs}" maxOccurs="1">{annotation}</xs:element>'
        elif t == 'number':
            min_occurs = '1' if name in (required_fields or []) else '0'
            return f'<xs:element name="{to_pascal_case(name)}" type="xs:decimal" minOccurs="{min_occurs}" maxOccurs="1">{annotation}</xs:element>'
        elif t == 'boolean':
            min_occurs = '1' if name in (required_fields or []) else '0'
            return f'<xs:element name="{to_pascal_case(name)}" type="xs:boolean" minOccurs="{min_occurs}" maxOccurs="1">{annotation}</xs:element>'
        else:
            min_occurs = '1' if name in (required_fields or []) else '0'
            return f'<xs:element name="{to_pascal_case(name)}" type="xs:string" minOccurs="{min_occurs}" maxOccurs="1">{annotation}</xs:element>'
